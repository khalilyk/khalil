...
export default function WorkDashboard({ userId, items, currency }: {
  userId: string; items: WorkItem[]; currency: string
}) {
  ...
  return (
    <div className="grid gap-4 lg:grid-cols-3 [&>*]:min-w-0">
      {BUSINESSES.map(biz => {
        ...
        const revenue = mine.filter(i => i.kind === 'revenue').reduce((s, i) => s + (i.amount ?? 0), 0)
        const pending = mine.filter(i => (i.kind === 'project' || i.kind === 'order') && i.status !== 'done')
